 ...
 */
function trim(cellValue) {
    return cellValue.trim();
}
function upper(cellValue) {
    return cellValue.toUpperCase();
}
function removeDuplicates(dataRange) {
    const uniqueValues = [...new Set(dataRange)];
    return uniqueValues;
}
function findAndReplace(data, findValue, replaceValue) {
    return data.map(row => row.map(cell => cell.replace(findValue, replaceValue)));
}
let cellProperties = create2DArray(rowCount, colCount, {
    textAlign: "left",
    bold: false,
...
function setBold(row, col, isBold) {
    cellProperties[row][col].bold = isBold;
}

function setFontSize(row, col, size) {
    cellProperties[row][col].fontSize = size;
}
function addRow() {
    rowCount++;
    spreadsheetData.push(new Array(colCount).fill(""));
    rowHeights.push(26);
}
